...
def process(data):
	...
	sentence_embedding = torch.mean(layers[11], 1).cpu()

	media = torch.tensor([0., 0., 0.])
	if 'Thoto' in data[features_to_idx['present_media']]:
		media[0] = 1
	if 'Video' in data[features_to_idx['present_media']]:
		media[1] = 1
	if 'Gif' in data[features_to_idx['present_media']]:
		media[2] = 1

	tweet_type = torch.tensor([0., 0., 0., 0.])
	if "Retweet" == data[features_to_idx['tweet_type']]:
		tweet_type[0] = 1
	elif "Quote" == data[features_to_idx['tweet_type']]:
		tweet_type[1] = 1
	elif "Reply" == data[features_to_idx['tweet_type']]:
		tweet_type[2] = 1
	elif "Toplevel" == data[features_to_idx['tweet_type']]:
		tweet_type[3] = 1
